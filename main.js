...
const NOTCH_GRABBER_HEIGHT = 6;
// macOS 折叠主体仍与物理刘海重合，只额外保留 6px 透明点击区来显示抓手。
// 这样新款 MacBook 的黑色物理刘海不会吞掉唯一的展开提示。
...
const SCREEN_RECORDINGS_DIR_NAME = 'screen-recordings';
...
const SCREEN_RECORDING_MAX_BYTES = 500 * 1024 * 1024;
...
function getCollapsedHeight(display) {
  ...
  // 黑色主体占菜单栏高度；额外 6px 只用于在物理刘海下方显示抓手。
  // 异常取到 0 时仍以 38px 作为菜单栏高度兜底。
  return (mb > 0 ? mb : COLLAPSED_MIN_HEIGHT) + NOTCH_GRABBER_HEIGHT;
}
...
function createWindow() {
  ...
  // 录屏必须由渲染层中用户主动点击的 getDisplayMedia 发起。
  // 当前产品只录制主显示器，不捕获系统音频或麦克风。
  mainWindow.webContents.session.setDisplayMediaRequestHandler(async (_request, callback) => {
    try {
      ...
      callback(source ? { video: source, audio: 'none' } : {});
    } catch (error) {
      callback({});
    }
  });
...
function copyWorkspaceAssets(sourceRoot, targetRoot) {
  ...
  for (const directory of [RECORDINGS_DIR_NAME, SCREEN_RECORDINGS_DIR_NAME, CLIP_IMAGES_DIR_NAME]) {
...
function normalizePortableStorage(storage) {
  ...
  const normalizers = [
    ...
    ['notch-screen-recordings', 'videoPath', SCREEN_RECORDINGS_DIR_NAME],
  ];
...
function getLayoutMetrics(display) {
  const d = display || getWindowDisplay();
  return {
    stripHeight: getCollapsedHeight(d), // 原生折叠窗口总高（菜单栏高 + 6px 抓手区）
...
const LOCAL_TRANSLATION_PYTHON_CANDIDATES = process.platform === 'win32'
  ? [path.join(__dirname, '.translation-venv', 'Scripts', 'python.exe'), 'python', 'python3']
  : [path.join(__dirname, '.translation-venv', 'bin', 'python'), 'python3', 'python'];

const LOCAL_TRANSLATION_ENV = {
  ...process.env,
  XDG_DATA_HOME: app.isPackaged ? path.join(app.getPath('userData'), 'argos-data') : path.join(__dirname, '.argos-data'),
  XDG_CONFIG_HOME: app.isPackaged ? path.join(app.getPath('userData'), 'argos-config') : path.join(__dirname, '.argos-config'),
  XDG_CACHE_HOME: app.isPackaged ? path.join(app.getPath('userData'), 'argos-cache') : path.join(__dirname, '.argos-cache'),
};

function inferTranslationSource(text) {
...
function runLocalArgosTranslation(text, targetLanguage) {
  const sourceLanguage = inferTranslationSource(text);
  const target = targetLanguage === 'zh-CN' ? 'zh' : targetLanguage;
  if (sourceLanguage === target) return Promise.resolve({ ok: true, text, engine: 'local' });
  const script = [
    'import sys',
    'try:',
    '    from argostranslate import translate',
    'except Exception:',
    '    print("__ARGOS_UNAVAILABLE__")',
    '    raise SystemExit(2)',
    'source, target, text = sys.argv[1], sys.argv[2], sys.argv[3]',
    'try:',
    '    result = translate.translate(text, source, target)',
    'except Exception:',
    '    print("__ARGOS_MODEL_MISSING__")',
    '    raise SystemExit(3)',
    'print(result, end="")',
  ].join('\n');
  ...
  return new Promise((resolve) => {
    const tryNext = (index) => {
      const command = LOCAL_TRANSLATION_PYTHON_CANDIDATES[index];
      if (!command) {
        resolve({ ok: false, error: 'local_unavailable' });
        return;
      }
      execFile(command, ['-c', script, sourceLanguage, target, text], { timeout: 30000, maxBuffer: 256 * 1024, env: LOCAL_TRANSLATION_ENV }, (error, stdout, stderr) => {
        const output = String(stdout || '').trim();
        if (!error && output && !output.startsWith('__ARGOS_')) {
          resolve({ ok: true, text: output.slice(0, 12000), engine: 'local' });
          return;
        }
        ...
        resolve({
          ok: false,
          error: output === '__ARGOS_MODEL_MISSING__' ? 'local_model_missing' : 'local_unavailable',
          detail: String(stderr || '').slice(0, 300),
        });
      });
    };
    tryNext(0);
...
ipcMain.handle('tools:translation-status', async () => {
  for (const command of LOCAL_TRANSLATION_PYTHON_CANDIDATES) {
    const available = await new Promise((resolve) => {
      execFile(command, ['-c', 'import argostranslate'], { timeout: 3000, env: LOCAL_TRANSLATION_ENV }, (error) => resolve(!error));
    });
    if (available) return { localAvailable: true, command };
  }
  return { localAvailable: false };
});

ipcMain.handle('tools:translate', async (event, payload) => {
  const text = String(payload && payload.text || '').trim().slice(0, 5000);
  const targetLanguage = String(payload && payload.targetLanguage || 'en');
  const languages = { 'zh-CN': '简体中文', en: 'English', ja: '日本語', ko: '한국어' };
  if (!text) return { ok: false, error: 'empty_text' };
  if (!languages[targetLanguage]) return { ok: false, error: 'invalid_target' };
  const engine = ['auto', 'local', 'online'].includes(payload && payload.engine) ? payload.engine : 'auto';
  if (engine === 'local' || engine === 'auto') {
    const localResult = await runLocalArgosTranslation(text, targetLanguage);
    if (localResult.ok || engine === 'local') return localResult;
  }
  const config = resolveLlmConfig();
  ...
  try {
    ...
    if (base.protocol !== 'https:') return { ok: false, error: 'invalid_endpoint' };
    endpoint = base.pathname.replace(/\/+$/, '').endsWith('/chat/completions')
      ? base
      : new URL(`${base.pathname.replace(/\/+$/, '')}/chat/completions`.replace(/^([^/])/, '/$1'), base);
  } catch (error) {
  ...
  try {
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        ...(config.baseUrl.includes('deepseek.com') ? { thinking: { type: 'disabled' } } : {}),
        messages: [
          { role: 'system', content: `Translate the user's text into ${languages[targetLanguage]}. Output only the translation, with no explanation or heading.` },
    ...
    const translation = String(result?.choices?.[0]?.message?.content || '').trim().slice(0, 12000);
    return translation ? { ok: true, text: translation } : { ok: false, error: 'invalid_response' };
  } catch (error) {
...
async function sendNetEaseShortcut(action) {
  ...
  const shortcut = netEaseShortcutSpec(action);
  if (!shortcut) return { ok: false, error: 'invalid_action' };
  try {
    const result = await runJxa(NETEASE_SHORTCUT_JXA, [
      shortcut.keyCode,
      shortcut.command ? '1' : '0',
      shortcut.dismissOverlays ? '1' : '0',
    ]);
    return result === 'ok' ? { ok: true } : { ok: false, error: 'netease_control_failed' };
  } catch (error) {
    console.warn('[music] failed to send NetEase Music shortcut', error && error.message || error);
...
ipcMain.handle('music:control', async (event, action) => {
  ...
  const result = await controlNetEaseMusic(action, {
    ...
    sendShortcut: sendNetEaseShortcut,
  }, netEaseMusicPlaying);
...
// ============ 屏幕录制 ============
function getScreenRecordingsDir() {
  return workspacePath(SCREEN_RECORDINGS_DIR_NAME);
}

function ensureScreenRecordingsDir() {
  try {
    fs.mkdirSync(getScreenRecordingsDir(), { recursive: true });
  } catch (error) {}
}

function getSafeScreenRecordingPath(value) {
  if (typeof value !== 'string' || !value.trim()) return null;
  const directory = path.resolve(getScreenRecordingsDir());
  const resolvedPath = path.isAbsolute(value)
    ? path.resolve(value)
    : path.resolve(workspaceRoot(), value);
  if (path.dirname(resolvedPath) !== directory
    || !/^screen-recording-[a-z0-9-]+\.webm$/i.test(path.basename(resolvedPath))) return null;
  try {
    const directoryStat = fs.lstatSync(directory);
    const fileStat = fs.lstatSync(resolvedPath);
    return !directoryStat.isSymbolicLink() && directoryStat.isDirectory()
      && !fileStat.isSymbolicLink() && fileStat.isFile() ? resolvedPath : null;
  } catch (error) {
    return null;
  }
}

ipcMain.handle('screen-recordings:save', async (event, payload) => {
  if (!payload || !payload.bytes) return { ok: false, error: 'empty_video' };
  let buffer;
  try { buffer = Buffer.from(payload.bytes); } catch (error) { return { ok: false, error: 'invalid_video' }; }
  if (!buffer.length || buffer.length > SCREEN_RECORDING_MAX_BYTES) {
    return { ok: false, error: buffer.length ? 'video_too_large' : 'empty_video' };
  }
  ensureScreenRecordingsDir();
  const id = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 9)}`;
  const videoPath = path.join(getScreenRecordingsDir(), `screen-recording-${id}.webm`);
  try {
    await fs.promises.writeFile(videoPath, buffer, { flag: 'wx' });
    return { ok: true, videoPath: platformPolicy.portableMediaPath(SCREEN_RECORDINGS_DIR_NAME, videoPath), mimeType: 'video/webm' };
  } catch (error) {
    return { ok: false, error: 'write_failed' };
  }
});

ipcMain.handle('screen-recordings:read', async (event, videoPath) => {
  const safePath = getSafeScreenRecordingPath(videoPath);
  if (!safePath) return null;
  try { return { bytes: await fs.promises.readFile(safePath), mimeType: 'video/webm' }; } catch (error) { return null; }
});

ipcMain.handle('screen-recordings:delete', async (event, videoPath) => {
  const safePath = getSafeScreenRecordingPath(videoPath);
  if (!safePath) return false;
  try { await fs.promises.unlink(safePath); return true; } catch (error) { return false; }
});

ipcMain.handle('screen-recordings:reveal', (event, videoPath) => {
  const safePath = getSafeScreenRecordingPath(videoPath);
  if (!safePath) return false;
  shell.showItemInFolder(safePath);
  return true;
