...
TO-DO Panel 是一个常驻 macOS / Windows 屏幕顶部的本地工作台。Mac 默认折叠到物理刘海区域，并在下沿露出 6px 抓手；Windows 显示为贴顶的 200 × 38 逻辑像素悬浮条，避开顶部任务栏。点击后从顶部展开。

| 页面 | 解决什么问题 |
| --- | --- |
| **首页** | 当前窗口、镜子、快速录音、随笔记、常用指令、网易云音乐、番茄钟、计算器和翻译集中在一个可自由排布的 Bento 工作台 |
...
| **工具** | 本地四则计算器；翻译仅在主动点击时通过已保存的 DeepSeek / 兼容 API 请求 |
| **设置** | 当菜单栏图标被刘海遮挡时，仍可在面板内配置 API、镜子、首页组件、功能显示、默认展开页、快捷键、数据目录与开机启动 |

剪贴板历史默认关闭，可从菜单栏或面板「设置」的「显示功能」中按需启用。工具页默认开启：计算器完全离线运行；翻译只在点击「翻译」后把输入文字发给你自己配置的 DeepSeek / OpenAI-compatible 服务，未配置 API 时不会发送任何内容。菜单栏入口与设置页读写同一份本机配置；即使状态栏图标过多、被物理刘海遮挡，也不影响调整。Codex、Claude Code 与 GPT 的本机完成事件也可以直接显示为不抢焦点的顶部提醒。

「设置 → 首页组件」可以隐藏或恢复当前平台可用的组件（macOS 九个、Windows 七个），但首页至少保留一个。还可选择「自由排布 / 紧凑均衡 / 专注工作」三种排布方式；选择预设后仍保留组件显示开关，手动拖拽或调整组件尺寸会自动回到自由排布。隐藏后，其余组件会自动重新铺满整个 Bento 网格，不留空洞；组件数据、用户保存的排列顺序和尺寸偏好不会被删除或覆盖。只要存在隐藏组件，首页使用自动填充布局并暂时隐藏尺寸按钮；macOS 恢复全部七个组件后，原尺寸偏好与按钮会一并恢复。Windows 采用自动填充布局。该偏好随当前本地工作区保存，存储键为 `notch-home-hidden-modules-v1` 和 `notch-home-layout-preset-v1`。
...
Windows 首版暂不显示「当前窗口」和「网易云音乐」组件；剪贴板点击复制后用 Ctrl+V 粘贴，AI 完成提醒暂不支持点击切回任务窗口。平台限制不覆盖原有组件偏好。镜子仅主动点击后开启，离开首页或收起立即释放；结束录音释放麦克风。系统拒绝访问设备时，请检查 Windows 隐私设置中桌面应用的相机 / 麦克风权限。
...
- **贴顶但不打扰**：Mac 折叠态宽 200px，黑色主体跟随菜单栏高度，下沿额外露出 6px 透明抓手区；展开与通知都不使用系统窗口动画。
