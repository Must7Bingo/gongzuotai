...
const ALL_TABS = ['home', 'todo', 'notes', 'links', 'recordings', 'credentials', 'tools', 'clip', 'settings'];
const normalizeTopTab = (name) => name === 'clip' ? 'tools' : name;
...
let activeWorkbenchView = 'clip';
const workbenchViewButtons = Array.from(document.querySelectorAll('[data-workbench-view]'));
const workbenchViewPanels = Array.from(document.querySelectorAll('[data-workbench-panel]'));
function setWorkbenchView(view = 'clip') {
  const next = ['clip', 'calculator', 'translator'].includes(view) ? view : 'clip';
  activeWorkbenchView = next;
  workbenchViewButtons.forEach((button) => {
    const selected = button.dataset.workbenchView === next;
    button.classList.toggle('active', selected);
    button.setAttribute('aria-selected', String(selected));
  });
  workbenchViewPanels.forEach((panel) => {
    const selected = panel.dataset.workbenchPanel === next;
    panel.classList.toggle('active', selected);
    panel.hidden = !selected;
  });
  document.dispatchEvent(new CustomEvent('notch:workbenchchange', { detail: { view: next } }));
}
workbenchViewButtons.forEach((button) => {
...
function applyTabDom(name) {
  ...
  positionIndicator();
  requestAnimationFrame(() => requestAnimationFrame(positionIndicator));
  document.dispatchEvent(new CustomEvent('notch:tabchange', { detail: { tab: name, workbenchView: activeWorkbenchView } }));
}
...
async function setActiveTab(name) {
  ...
  if (requestedTab === 'clip') setWorkbenchView('clip');
  if (tabBusy) {
  ...
  try {
    ...
    const runHeavyLoads = () => {
      if (_tabNameForDeferred === 'tools' && activeWorkbenchView === 'clip') renderClipList();
...
function keyEventToAccelerator(event) {
  ...
  let key = event.code === 'Backquote' ? 'Backquote' : (keyAliases[event.key] || event.key);
  if (/^[a-z]$/i.test(key)) key = key.toUpperCase();
  if (!/^(?:[A-Z0-9]|F(?:[1-9]|1[0-9]|2[0-4])|Space|Tab|Escape|Left|Right|Up|Down|Home|End|PageUp|PageDown|Backspace|Delete|Enter|Backquote)$/.test(key)) return '';
  const parts = [];
...
document.addEventListener('notch:workbenchchange', (event) => {
  if (event.detail?.view === 'clip' && activeTab === 'tools') renderClipList();
  if (event.detail?.view !== 'clip') stopScreenRecording({ discard: true });
});
...
const HOME_SIZES_KEY = 'notch-home-widget-sizes-v3';
const HOME_HIDDEN_MODULES_KEY = 'notch-home-hidden-modules-v1';
const HOME_MODULE_REGISTRY = ['music', 'pomodoro', 'recorder', 'windows', 'mirror', 'note', 'commands', 'calculator', 'translator', 'clip'];
...
const HOME_ORDER_DEFAULTS = ['music', 'pomodoro', 'recorder', 'windows', 'mirror', 'note', 'commands', 'calculator', 'translator', 'clip'];
...
const HOME_LAYOUT_PRESET_KEY = 'notch-home-layout-preset-v1';
const HOME_LAYOUT_PRESETS = {
  compact: {
    order: ['music', 'pomodoro', 'recorder', 'calculator', 'clip', 'windows', 'mirror', 'note', 'commands', 'translator'],
    sizes: { music: 'mini', pomodoro: 'mini', recorder: 'small', calculator: 'small', windows: 'medium', mirror: 'medium', note: 'medium', commands: 'small', translator: 'medium', clip: 'small' },
  },
  focus: {
    order: ['windows', 'note', 'translator', 'mirror', 'recorder', 'calculator', 'clip', 'music', 'pomodoro', 'commands'],
    sizes: { music: 'mini', pomodoro: 'mini', recorder: 'small', calculator: 'small', windows: 'medium', mirror: 'medium', note: 'medium', commands: 'small', translator: 'medium', clip: 'small' },
  },
};

function loadHomeLayoutPreset() {
  const preset = localStorage.getItem(HOME_LAYOUT_PRESET_KEY);
  return preset === 'compact' || preset === 'focus' ? preset : 'free';
}
...
function loadHomeOrder() {
  try {
    ...
    if (
      Array.isArray(saved)
      && (saved.length === HOME_ORDER_DEFAULTS.length || saved.length === HOME_ORDER_DEFAULTS.length - 1)
      && new Set(saved).size === saved.length
      && saved.every((id) => HOME_ORDER_DEFAULTS.includes(id))
    ) return saved.includes('clip') ? saved : [...saved, 'clip'];
...
let homeLayoutPreset = loadHomeLayoutPreset();
...
function saveHomeLayout() {
  try {
    ...
    localStorage.setItem(HOME_LAYOUT_PRESET_KEY, homeLayoutPreset);
    return true;
  } catch (error) {
...
function notifyHomeLayoutPresetChanged(persisted) {
  document.dispatchEvent(new CustomEvent('notch:home-layout-preset-changed', {
    detail: { preset: homeLayoutPreset, persisted },
  }));
}

function setHomeLayoutPreset(preset) {
  const nextPreset = String(preset || 'free');
  if (!['free', ...Object.keys(HOME_LAYOUT_PRESETS)].includes(nextPreset)) {
    return { ok: false, error: 'invalid_preset', preset: homeLayoutPreset };
  }
  if (homeLayoutReadOnly) return { ok: false, error: 'layout_read_only', preset: homeLayoutPreset };
  if (nextPreset === 'free') {
    homeLayoutPreset = 'free';
    const persisted = saveHomeLayout();
    notifyHomeLayoutPresetChanged(persisted);
    return { ok: true, preset: homeLayoutPreset, persisted };
  }
  const presetLayout = HOME_LAYOUT_PRESETS[nextPreset];
  const nextOrder = [...presetLayout.order];
  const nextSizes = { ...presetLayout.sizes };
  const layout = resolveValidatedHomeLayout(hiddenHomeModules, nextOrder, nextSizes);
  if (!layout) return { ok: false, error: 'layout_invalid', preset: homeLayoutPreset };
  const previousOrder = homeOrder;
  const previousSizes = homeSizes;
  const previousPreset = homeLayoutPreset;
  try {
    homeOrder = nextOrder;
    homeSizes = nextSizes;
    homeLayoutPreset = nextPreset;
    applyHomeLayout(layout, { reason: 'preset' });
  } catch (error) {
    homeOrder = previousOrder;
    homeSizes = previousSizes;
    homeLayoutPreset = previousPreset;
    try { applyHomeLayout(resolveValidatedHomeLayout(hiddenHomeModules), { reason: 'rollback' }); } catch (rollbackError) {}
    return { ok: false, error: 'dom_apply_failed', preset: previousPreset };
  }
  const persisted = saveHomeLayout();
  notifyHomeLayoutPresetChanged(persisted);
  return { ok: true, preset: homeLayoutPreset, persisted };
...
window.NotchHome = Object.freeze({
  ...
  getLayoutPreset: () => homeLayoutPreset,
  setLayoutPreset: setHomeLayoutPreset,
});
...
if (homeBento) {
  ...
  const finishHomeDrag = (event, cancelled = false) => {
    ...
    if (!cancelled && target && target !== tile) {
      ...
      if (layout) {
        const presetChanged = homeLayoutPreset !== 'free';
        homeLayoutPreset = 'free';
        applyHomeLayout(layout, { reason: 'reorder' });
        const persisted = saveHomeLayout();
        if (presetChanged) notifyHomeLayoutPresetChanged(persisted);
...
// ============ 首页 · 统一液态指针反馈 ============
function bindHomeLiquidFeedback() {
  if (!homeBento || !homeTiles.length) return;
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)');
  const states = new WeakMap();

  const ensureSurface = (tile) => {
    let surface = tile.querySelector(':scope > .tile-liquid-surface');
    if (surface) return surface;
    surface = document.createElement('span');
    surface.className = 'tile-liquid-surface';
    surface.setAttribute('aria-hidden', 'true');
    tile.prepend(surface);
    return surface;
  };

  const clearTile = (tile, state) => {
    if (state?.frame) cancelAnimationFrame(state.frame);
    tile.classList.remove('liquid-hover', 'liquid-ripple');
    tile.style.setProperty('--tile-liquid-shift-x', '0px');
    tile.style.setProperty('--tile-liquid-shift-y', '0px');
    tile.style.setProperty('--tile-liquid-stretch-x', '1');
    tile.style.setProperty('--tile-liquid-stretch-y', '1');
  };

  const updateTile = (tile, state) => {
    state.frame = null;
    if (!state.pending || reducedMotion.matches || homeBento.classList.contains('layout-dragging')) return;
    const { clientX, clientY } = state.pending;
    ...
    const deltaX = state.lastPoint ? clientX - state.lastPoint.x : 0;
    const deltaY = state.lastPoint ? clientY - state.lastPoint.y : 0;
    const speed = Math.min(18, Math.hypot(deltaX, deltaY));
    const stretch = 1 + speed * .012;
    tile.style.setProperty('--tile-liquid-x', `${(x * 100).toFixed(2)}%`);
    tile.style.setProperty('--tile-liquid-y', `${(y * 100).toFixed(2)}%`);
    tile.style.setProperty('--tile-liquid-shift-x', `${Math.max(-7, Math.min(7, deltaX * .28)).toFixed(2)}px`);
    tile.style.setProperty('--tile-liquid-shift-y', `${Math.max(-7, Math.min(7, deltaY * .28)).toFixed(2)}px`);
    tile.style.setProperty('--tile-liquid-stretch-x', stretch.toFixed(3));
    tile.style.setProperty('--tile-liquid-stretch-y', (1 + speed * .004).toFixed(3));
    state.lastPoint = { x: clientX, y: clientY };
  };

  const playRipple = (tile) => {
    tile.classList.remove('liquid-ripple');
    void tile.offsetWidth;
    tile.classList.add('liquid-ripple');
  };

  homeTiles.forEach((tile) => {
    ensureSurface(tile);
    const state = { frame: null, pending: null, lastPoint: null };
    states.set(tile, state);
    tile.addEventListener('pointerenter', (event) => {
      if (reducedMotion.matches || homeBento.classList.contains('layout-dragging')) return;
      tile.classList.add('liquid-hover');
      state.pending = event;
    ...
    tile.addEventListener('pointermove', (event) => {
      if (reducedMotion.matches || homeBento.classList.contains('layout-dragging')) return;
      state.pending = event;
      if (!state.frame) state.frame = requestAnimationFrame(() => updateTile(tile, state));
    });
    tile.addEventListener('pointerdown', (event) => {
      if (event.button !== 0 || reducedMotion.matches || homeBento.classList.contains('layout-dragging')) return;
      state.pending = event;
      updateTile(tile, state);
      playRipple(tile);
    });
    tile.addEventListener('pointerleave', () => {
      ...
      clearTile(tile, state);
    });
  });

  reducedMotion.addEventListener('change', () => {
    if (!reducedMotion.matches) return;
    homeTiles.forEach((tile) => clearTile(tile, states.get(tile)));
  });
}

bindHomeLiquidFeedback();
...
if (mirrorStage) {
  // 首页悬停视觉统一由 bindHomeLiquidFeedback 驱动；镜子仅保留摄像头功能交互。
...
// ============ 首页 · 最近剪贴板组件 ============
const HOME_CLIP_LIMIT = 10;
const homeClipList = document.getElementById('home-clip-list');
const homeClipCount = document.getElementById('home-clip-count');
const homeClipOpen = document.getElementById('home-clip-open');

function getHomeClipEntries() {
  return Array.isArray(clipHistory) ? clipHistory.slice(0, HOME_CLIP_LIMIT) : [];
}

function homeClipDetails(entry) {
  const isImage = entry.type === 'image';
  const isUrl = entry.type === 'url' || (!isImage && CLIP_URL_RE.test(String(entry.text || '').trim()));
  const text = isImage ? '图片记录' : String(entry.text || '').replace(/\s+/g, ' ').trim();
  return {
    isImage,
    isUrl,
    label: isImage ? '图片' : isUrl ? '链接' : '文字',
    preview: text || '空白文字',
  };
}

function homeClipRowHtml(entry) {
  const details = homeClipDetails(entry);
  const safeId = escapeHtml(entry.id);
  const imageDataUrl = details.isImage && entry.imagePath ? clipImageCache.get(entry.imagePath) : '';
  const thumbnail = details.isImage
    ? (imageDataUrl
      ? `<img src="${escapeHtml(imageDataUrl)}" alt="" draggable="false" />`
      : '<span class="home-clip-row-image">图</span>')
    : `<span class="home-clip-row-type">${details.isUrl ? '↗' : 'T'}</span>`;
  const favoriteLabel = clipFavorites.includes(entry.id) ? '取消收藏' : '收藏';
  const favoriteMark = clipFavorites.includes(entry.id) ? '★' : '☆';
  return `<article class="home-clip-row${details.isImage ? ' is-image' : ''}" data-id="${safeId}">
    <button class="home-clip-row-copy" type="button" data-home-clip-action="copy" data-id="${safeId}" aria-label="复制${escapeHtml(details.preview.slice(0, 80))}">
      <span class="home-clip-row-thumb" aria-hidden="true">${thumbnail}</span>
      <span class="home-clip-row-copy-area"><span class="home-clip-row-preview">${escapeHtml(details.preview)}</span><span class="home-clip-row-meta">${details.label} · ${escapeHtml(formatClipTime(entry.timestamp))}</span></span>
      <span class="home-clip-row-copy-hint" aria-hidden="true">↩</span>
    </button>
    <span class="home-clip-row-actions">
      <button class="home-clip-row-action${clipFavorites.includes(entry.id) ? ' faved' : ''}" type="button" data-home-clip-action="favorite" data-id="${safeId}" aria-label="${favoriteLabel}">${favoriteMark}</button>
      <button class="home-clip-row-action delete" type="button" data-home-clip-action="delete" data-id="${safeId}" aria-label="删除这条记录">×</button>
    </span>
  </article>`;
}

function renderHomeClip() {
  if (!homeClipList) return;
  const entries = getHomeClipEntries();
  if (homeClipCount) homeClipCount.textContent = `最近 ${entries.length} 条`;
  if (!entries.length) {
    homeClipList.innerHTML = '<div class="home-clip-empty">还没有剪贴板记录<br /><span>复制内容后，会在这里保留最近 10 条</span></div>';
    return;
  }

  homeClipList.innerHTML = entries.map(homeClipRowHtml).join('');
  const missingImagePaths = entries
    .filter((entry) => entry.type === 'image' && entry.imagePath && !clipImageCache.has(entry.imagePath))
    .map((entry) => entry.imagePath);
  if (missingImagePaths.length) {
    Promise.all(missingImagePaths.map((imagePath) => preloadClipImage(imagePath))).then(() => renderHomeClip());
  }
}

homeClipList?.addEventListener('click', async (event) => {
  event.stopPropagation();
  const actionButton = event.target.closest('[data-home-clip-action]');
  if (!actionButton) return;
  const { homeClipAction: action, id } = actionButton.dataset;
  if (!id) return;

  if (action === 'copy') {
    if (await copyClipEntry(id)) {
      actionButton.closest('.home-clip-row')?.classList.add('copied');
      setTimeout(() => actionButton.closest('.home-clip-row')?.classList.remove('copied'), 800);
    }
    return;
  }
  if (action === 'favorite') {
  ...
  if (action === 'delete') deleteClipEntry(id);
});

homeClipOpen?.addEventListener('click', (event) => { event.stopPropagation(); setActiveTab('clip'); });
...
let selectedClipId = clipHistory[0]?.id || null;
...
const clipDetailEl = document.getElementById('clip-detail');
...
function clipTypeIcon(type, imageDataUrl = '') {
  if (type === 'image') {
    return imageDataUrl
      ? `<img src="${escapeHtml(imageDataUrl)}" alt="" draggable="false"/>`
      : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="4" y="5" width="16" height="14" rx="2"/><circle cx="9" cy="10" r="1.2"/><path d="m5 17 4-4 3 3 3-3 4 4"/></svg>';
  }
  if (type === 'url') {
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.1.1l2-2a5 5 0 0 0-7.1-7.1l-1.1 1.1"/><path d="M14 11a5 5 0 0 0-7.1-.1l-2 2A5 5 0 0 0 12 20l1.1-1.1"/></svg>';
  }
  return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 7h12M6 12h9M6 17h6"/></svg>';
}

function clipTextDetails(entry) {
  const raw = (entry.text || '').replace(/\s+/g, ' ').trim();
  const isUrl = entry.type === 'url' || CLIP_URL_RE.test(raw);
  if (isUrl) {
    try {
      const url = new URL(raw);
      return { type: 'url', title: url.hostname || '链接', preview: raw || '公开链接', label: '复制链接' };
    } catch (error) {}
  }
  const title = raw.slice(0, 72) || '空白文字';
  const rest = raw.slice(72, 160);
  return {
    type: 'text',
    title,
    preview: rest || `文字 · ${raw.length} 个字符`,
    label: `复制：${raw.slice(0, 80) || '空白内容'}`,
  };
}
...
function clipEntryHtml(entry, faved) {
  ...
  const safeId = escapeHtml(entry.id);
  const imageDataUrl = entry.type === 'image' && entry.imagePath ? clipImageCache.get(entry.imagePath) : '';
  const details = entry.type === 'image'
    ? { type: 'image', title: '图片', preview: imageDataUrl ? '本机已保存 · 点击复制' : '图片正在加载', label: '复制图片' }
    : clipTextDetails(entry);
  const title = escapeHtml(details.title);
  const preview = escapeHtml(details.preview);
  const icon = clipTypeIcon(details.type, imageDataUrl);

  return `<div class="clip-item clip-item-${details.type} clip-type-${details.type}${entry.id === selectedClipId ? ' selected' : ''}" data-id="${safeId}" data-type="${details.type}">
  <button class="clip-copy-target" type="button" data-action="copy" aria-label="${escapeHtml(details.label)}">
    <span class="clip-type-icon">${icon}</span>
    <span class="clip-entry-copy"><span class="clip-entry-title">${title}</span><span class="clip-entry-preview">${preview}</span></span>
    <span class="clip-entry-tail"><time class="clip-time">${timeStr}</time><span class="clip-paste-glyph" aria-hidden="true">↩</span></span>
  </button>
  <span class="clip-entry-actions">
    <button class="clip-fav-btn${favClass}" type="button" data-action="fav" aria-label="${favLabel}">${star}</button>
    <button class="clip-del-btn" type="button" data-action="delete" aria-label="删除"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true"><path d="m7 7 10 10M17 7 7 17"/></svg></button>
  </span>
</div>`;
}
...
function renderClipDetail() {
  if (!clipDetailEl) return;
  const entry = clipHistory.find((item) => item.id === selectedClipId) || getFilteredClipItems()[0] || null;
  if (!entry) { clipDetailEl.innerHTML = '<div class="clip-detail-empty">从左侧选择一条记录，在这里查看完整内容。</div>'; return; }
  if (entry.id !== selectedClipId) selectedClipId = entry.id;
  const faved = clipFavorites.includes(entry.id);
  const safeId = escapeHtml(entry.id);
  const time = escapeHtml(formatClipTime(entry.timestamp));
  if (entry.type === 'image') {
    const dataUrl = entry.imagePath ? clipImageCache.get(entry.imagePath) : '';
    clipDetailEl.innerHTML = `<header class="clip-detail-head"><span><b>图片预览</b><small>${time}</small></span><div><button type="button" data-detail-action="copy" data-id="${safeId}">复制</button><button type="button" data-detail-action="fav" data-id="${safeId}">${faved ? '已收藏' : '收藏'}</button></div></header><div class="clip-preview-scroll">${dataUrl ? `<img src="${escapeHtml(dataUrl)}" alt="剪贴板图片完整预览" draggable="false" />` : '<span class="clip-preview-loading">正在加载图片预览…</span>'}</div>`;
    if (!dataUrl && entry.imagePath) preloadClipImage(entry.imagePath).then(() => renderClipDetail());
    return;
  }
  const text = String(entry.text || '');
  const isUrl = entry.type === 'url' || CLIP_URL_RE.test(text.trim());
  clipDetailEl.innerHTML = `<header class="clip-detail-head"><span><b>${isUrl ? '链接详情' : '文字详情'}</b><small>${time}</small></span><div><button type="button" data-detail-action="copy" data-id="${safeId}">复制</button><button type="button" data-detail-action="fav" data-id="${safeId}">${faved ? '已收藏' : '收藏'}</button></div></header><pre class="clip-text-preview">${escapeHtml(text || '（空白文字）')}</pre>`;
}

function renderClipList() {
  if (!clipListEl) return;
  // 脏标记：数据/过滤器/图片缓存均未变则跳过全量重建
  if (clipDataVersion === lastRenderedClipVersion) { renderClipDetail(); return; }

  const items = getFilteredClipItems();
  const favSet = new Set(clipFavorites);
  if (clipCountEl) clipCountEl.textContent = String(clipHistory.length);
...
if (clipListEl) {
  clipListEl.addEventListener('click', (e) => {
    ...
    const id = item.dataset.id;
    if (!id) return;
    if (selectedClipId !== id) { selectedClipId = id; clipDataVersion++; renderClipList(); }
...
if (clipDetailEl) {
  clipDetailEl.addEventListener('click', (event) => {
    const button = event.target.closest('[data-detail-action]');
    if (!button) return;
    const id = button.dataset.id;
    if (button.dataset.detailAction === 'copy') copyClipEntry(id);
    if (button.dataset.detailAction === 'fav') toggleClipFavorite(id);
  });
}
...
function deleteClipEntry(id, focusContext = null) {
  ...
  if (selectedClipId === id) selectedClipId = clipHistory[0]?.id || null;
...
// ============ 屏幕录制（仅主屏、无系统音频） ============
const SCREEN_RECORDINGS_KEY = 'notch-screen-recordings';
const screenRecordStart = document.getElementById('screen-record-start');
const screenRecordStop = document.getElementById('screen-record-stop');
const screenRecorderStatus = document.getElementById('screen-recorder-status');
const screenRecordingPreview = document.getElementById('screen-recording-preview');
let screenRecordingStream = null;
let screenMediaRecorder = null;
let screenRecordingChunks = [];
let screenRecordingStartedAt = 0;
let screenRecordingStopping = false;
let screenPreviewUrl = '';
let screenRecordings = loadScreenRecordings();

function loadScreenRecordings() {
  try {
    const rows = JSON.parse(localStorage.getItem(SCREEN_RECORDINGS_KEY) || '[]');
    return Array.isArray(rows) ? rows.filter((row) => row && typeof row.videoPath === 'string').slice(0, 30) : [];
  } catch (error) { return []; }
}

function saveScreenRecordings() {
  try { localStorage.setItem(SCREEN_RECORDINGS_KEY, JSON.stringify(screenRecordings.slice(0, 30))); } catch (error) {}
}

function setScreenRecorderStatus(message, error = false) {
  if (!screenRecorderStatus) return;
  screenRecorderStatus.textContent = message;
  screenRecorderStatus.classList.toggle('error', error);
}

function syncScreenRecorderControls(recording) {
  if (screenRecordStart) screenRecordStart.disabled = recording;
  if (screenRecordStop) screenRecordStop.disabled = !recording;
}

function releaseScreenRecordingStream() {
  if (screenRecordingStream) screenRecordingStream.getTracks().forEach((track) => track.stop());
  screenRecordingStream = null;
}

function clearScreenPreviewUrl() {
  if (screenPreviewUrl) URL.revokeObjectURL(screenPreviewUrl);
  screenPreviewUrl = '';
}

function showScreenRecordingPreview(blob, metadata) {
  if (!screenRecordingPreview) return;
  clearScreenPreviewUrl();
  screenPreviewUrl = URL.createObjectURL(blob);
  const duration = Math.max(1, Math.round((metadata?.durationMs || 0) / 1000));
  screenRecordingPreview.hidden = false;
  screenRecordingPreview.innerHTML = `<video controls muted playsinline src="${screenPreviewUrl}" aria-label="最新屏幕录制"></video><div><span>最新录制 · ${duration} 秒</span><button type="button" data-screen-recording-reveal>在访达中显示</button></div>`;
}

async function renderLatestScreenRecording() {
  const latest = screenRecordings[0];
  if (!latest || !window.notchAPI?.readScreenRecording) return;
  const result = await window.notchAPI.readScreenRecording(latest.videoPath).catch(() => null);
  if (!result?.bytes) return;
  showScreenRecordingPreview(new Blob([result.bytes], { type: result.mimeType || 'video/webm' }), latest);
}

function preferredScreenRecordingMimeType() {
  return ['video/webm;codecs=vp9', 'video/webm;codecs=vp8', 'video/webm']
    .find((type) => window.MediaRecorder?.isTypeSupported?.(type)) || '';
}

async function stopScreenRecording({ discard = false } = {}) {
  const recorder = screenMediaRecorder;
  if (!recorder || screenRecordingStopping) return;
  screenRecordingStopping = true;
  if (discard) screenRecordingChunks = [];
  if (recorder.state !== 'inactive') recorder.stop();
}

async function startScreenRecording() {
  if (screenMediaRecorder || !navigator.mediaDevices?.getDisplayMedia || !window.notchAPI?.saveScreenRecording) {
    setScreenRecorderStatus('当前环境不支持屏幕录制。', true);
    return;
  }
  try {
    setScreenRecorderStatus('正在请求系统录屏权限…');
    const stream = await navigator.mediaDevices.getDisplayMedia({
      video: { frameRate: { ideal: 30, max: 30 } },
      audio: false,
    });
    const mimeType = preferredScreenRecordingMimeType();
    screenRecordingStream = stream;
    screenRecordingChunks = [];
    screenRecordingStartedAt = Date.now();
    screenRecordingStopping = false;
    screenMediaRecorder = mimeType ? new MediaRecorder(stream, { mimeType }) : new MediaRecorder(stream);
    screenMediaRecorder.addEventListener('dataavailable', (event) => {
      if (event.data?.size) screenRecordingChunks.push(event.data);
    });
    screenMediaRecorder.addEventListener('stop', async () => {
      const chunks = screenRecordingChunks;
      const durationMs = Date.now() - screenRecordingStartedAt;
      const finalMimeType = screenMediaRecorder?.mimeType || mimeType || 'video/webm';
      screenMediaRecorder = null;
      releaseScreenRecordingStream();
      syncScreenRecorderControls(false);
      if (!chunks.length) {
        setScreenRecorderStatus('录制已取消。');
        screenRecordingStopping = false;
        return;
      }
      const blob = new Blob(chunks, { type: finalMimeType });
      screenRecordingChunks = [];
      setScreenRecorderStatus('正在保存本机视频…');
      const saved = await window.notchAPI.saveScreenRecording({ bytes: await blob.arrayBuffer(), mimeType: finalMimeType }).catch(() => null);
      if (!saved?.ok) {
        setScreenRecorderStatus('保存失败，请检查可用磁盘空间。', true);
        screenRecordingStopping = false;
        return;
      }
      const metadata = { id: generateId(), videoPath: saved.videoPath, mimeType: saved.mimeType || 'video/webm', durationMs, createdAt: Date.now() };
      screenRecordings = [metadata, ...screenRecordings.filter((item) => item.videoPath !== metadata.videoPath)].slice(0, 30);
      saveScreenRecordings();
      showScreenRecordingPreview(blob, metadata);
      setScreenRecorderStatus('已保存到本机。');
      screenRecordingStopping = false;
    }, { once: true });
    stream.getVideoTracks()[0]?.addEventListener('ended', () => stopScreenRecording());
    screenMediaRecorder.start(1000);
    syncScreenRecorderControls(true);
    setScreenRecorderStatus('正在录制主屏…');
  } catch (error) {
    releaseScreenRecordingStream();
    screenMediaRecorder = null;
    syncScreenRecorderControls(false);
    setScreenRecorderStatus('未开始录制，请在系统设置中允许“屏幕录制”后重试。', true);
  }
}

screenRecordStart?.addEventListener('click', startScreenRecording);
screenRecordStop?.addEventListener('click', () => stopScreenRecording());
screenRecordingPreview?.addEventListener('click', (event) => {
  if (!event.target.closest('[data-screen-recording-reveal]')) return;
  const latest = screenRecordings[0];
  if (latest?.videoPath) window.notchAPI?.revealScreenRecording?.(latest.videoPath);
});
document.addEventListener('notch:tabchange', (event) => {
  if (event.detail?.tab !== 'tools' || event.detail?.workbenchView !== 'clip') stopScreenRecording({ discard: true });
});
document.addEventListener('notch:modechange', (event) => {
  if (!event.detail?.expanded) stopScreenRecording({ discard: true });
});
window.addEventListener('beforeunload', () => { releaseScreenRecordingStream(); clearScreenPreviewUrl(); });
renderLatestScreenRecording();
