(function exposeNotchDomain(root, factory) {
  ...
  const HOME_GAPLESS_TEMPLATES = {
    ...
    6: [
      { column: 0, row: 0, width: 4, height: 2 }, { column: 4, row: 0, width: 4, height: 2 }, { column: 8, row: 0, width: 4, height: 2 },
      { column: 0, row: 2, width: 4, height: 2 }, { column: 4, row: 2, width: 4, height: 2 }, { column: 8, row: 2, width: 4, height: 2 },
    ],
    7: [
      { column: 0, row: 0, width: 4, height: 1 }, { column: 4, row: 0, width: 4, height: 1 }, { column: 8, row: 0, width: 4, height: 1 },
      { column: 0, row: 1, width: 3, height: 3 }, { column: 3, row: 1, width: 3, height: 3 }, { column: 6, row: 1, width: 3, height: 3 }, { column: 9, row: 1, width: 3, height: 3 },
    ],
    8: [
      { column: 0, row: 0, width: 3, height: 1 }, { column: 3, row: 0, width: 3, height: 1 }, { column: 6, row: 0, width: 3, height: 1 }, { column: 9, row: 0, width: 3, height: 1 },
      { column: 0, row: 1, width: 3, height: 3 }, { column: 3, row: 1, width: 3, height: 3 }, { column: 6, row: 1, width: 3, height: 3 }, { column: 9, row: 1, width: 3, height: 3 },
    ],
  };
  ...
  function resolveHomeWidgetLayout(order, sizes, hiddenIds, columns = 12, rows = 4) {
    ...
    let placements;
    if (visibleOrder.length === ids.length && visibleOrder.length >= 7) {
  ...
  function calculateExpression(expression, options = {}) {
    const angleMode = String(options?.angleMode || 'deg').toLowerCase() === 'rad' ? 'rad' : 'deg';
    ...
    const tokens = [];
    const functionNames = new Set(['sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'log', 'ln', 'sqrt']);
    for (let index = 0; index < source.length;) {
      ...
      if (/[0-9.]/.test(char)) {
        ...
        if (name === 'e') tokens.push({ type: 'constant', value: Math.E });
        else if (functionNames.has(name)) tokens.push({ type: 'function', name });
    ...
    const startsPrimary = (token) => Boolean(token && ['number', 'constant', 'function', '('].includes(token.type));
    const finite = (value) => {
      if (!Number.isFinite(value)) throw new Error('overflow');
      return value;
    };
    const degreesToRadians = (value) => angleMode === 'deg' ? value * Math.PI / 180 : value;
    const radiansToDegrees = (value) => angleMode === 'deg' ? value * 180 / Math.PI : value;
    const applyFunction = (name, value) => {
      switch (name) {
        case 'sin': return finite(Math.sin(degreesToRadians(value)));
        case 'cos': return finite(Math.cos(degreesToRadians(value)));
        case 'tan': return finite(Math.tan(degreesToRadians(value)));
        case 'asin':
          if (value < -1 || value > 1) throw new Error('domain_error');
          return finite(radiansToDegrees(Math.asin(value)));
        case 'acos':
          if (value < -1 || value > 1) throw new Error('domain_error');
          return finite(radiansToDegrees(Math.acos(value)));
        case 'atan': return finite(radiansToDegrees(Math.atan(value)));
        case 'sqrt':
    ...
    function parsePrimary() {
      ...
      if (token.type === 'function') {
        position += 1;
        if (next()?.type !== '(') throw new Error('invalid');
        position += 1;
    ...
    function parseMultiplicative() {
      ...
      while (next()?.type === '*' || next()?.type === '/' || startsPrimary(next())) {
        const implicit = startsPrimary(next());
        const operator = implicit ? '*' : tokens[position++].type;
    ...
    try {
      const value = parseAdditive();
      if (position !== tokens.length || !Number.isFinite(value)) return { ok: false, error: 'invalid' };
      const normalized = Number(value.toPrecision(12));
      return { ok: true, value: normalized, display: String(normalized) };
    } catch (error) {
      const expectedErrors = new Set(['divide_by_zero', 'domain_error', 'overflow']);
      return { ok: false, error: expectedErrors.has(error.message) ? error.message : 'invalid' };
    }
  }
