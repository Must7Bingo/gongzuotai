(function initWorkspace() {
  ...
  const CALCULATOR_ANGLE_MODE_KEY = 'notch-calculator-angle-mode-v1';
  ...
  const settingsHomeLayoutPreset = document.getElementById('settings-home-layout-preset');
  ...
  const calculatorAngleButtons = Array.from(document.querySelectorAll('[data-calculator-angle]'));
  const calculatorAngleHint = document.getElementById('calculator-angle-hint');
  ...
  const translatorEngine = document.getElementById('translator-engine');
  ...
  const translatorResultMeta = document.getElementById('translator-result-meta');
  const translatorCopy = document.getElementById('translator-copy');
  const homeCalculatorInput = document.getElementById('home-calculator-input');
  const homeCalculatorResult = document.getElementById('home-calculator-result');
  const homeCalculatorSubmit = document.getElementById('home-calculator-submit');
  const homeTranslatorSource = document.getElementById('home-translator-source');
  const homeTranslatorTarget = document.getElementById('home-translator-target');
  const homeTranslatorEngine = document.getElementById('home-translator-engine');
  const homeTranslatorSubmit = document.getElementById('home-translator-submit');
  const homeTranslatorStatus = document.getElementById('home-translator-status');
  const homeTranslatorResult = document.getElementById('home-translator-result');

  let calculatorInput = '';
  let calculatorLastValue = '';
  let calculatorAngleMode = localStorage.getItem(CALCULATOR_ANGLE_MODE_KEY) === 'rad' ? 'rad' : 'deg';
  let translatorText = '';
  ...
  function renderSettingsPanel() {
    ...
    if (settingsDefaultTab) {
      const visibleTabs = new Set(Domain.visiblePanelTabs(
        ['home', 'todo', 'notes', 'links', 'recordings', 'credentials', 'tools', 'clip', 'settings'],
  ...
  function renderHomeModuleSettings() {
    ...
    if (settingsHomeLayoutPreset) {
      settingsHomeLayoutPreset.value = state?.layoutPreset || window.NotchHome?.getLayoutPreset?.() || 'free';
      settingsHomeLayoutPreset.disabled = state?.readOnly === true;
    }
  ...
  function calculatorErrorMessage(error) {
    if (error === 'divide_by_zero') return '不能除以 0';
    if (error === 'domain_error') return '定义域错误';
    if (error === 'overflow') return '结果超出范围';
    return '…';
  }

  function renderCalculator(result = null) {
    const display = !calculatorInput ? '0' : result?.ok ? result.display : calculatorErrorMessage(result?.error);
    if (calculatorExpression) calculatorExpression.textContent = calculatorInput || '输入算式';
    if (calculatorResult) calculatorResult.textContent = display;
    if (homeCalculatorResult) homeCalculatorResult.textContent = display;
    if (homeCalculatorInput && document.activeElement !== homeCalculatorInput) homeCalculatorInput.value = calculatorInput;
    calculatorAngleButtons.forEach((button) => {
      const active = button.dataset.calculatorAngle === calculatorAngleMode;
      button.classList.toggle('active', active);
      button.setAttribute('aria-pressed', String(active));
    });
    if (calculatorAngleHint) calculatorAngleHint.textContent = `三角函数按 ${calculatorAngleMode.toUpperCase()} 计算`;
  }

  function calculateCurrentExpression() {
    return Domain.calculateExpression(calculatorInput, { angleMode: calculatorAngleMode });
  }

  function setCalculatorAngleMode(angleMode) {
    calculatorAngleMode = angleMode === 'rad' ? 'rad' : 'deg';
    try { localStorage.setItem(CALCULATOR_ANGLE_MODE_KEY, calculatorAngleMode); } catch (error) {}
    const result = calculatorInput ? calculateCurrentExpression() : null;
    renderCalculator(result?.ok ? result : result);
  }

  function evaluateCalculator() {
    if (!calculatorInput) { renderCalculator(); return; }
    const result = calculateCurrentExpression();
    renderCalculator(result);
    if (result.ok) calculatorLastValue = result.display;
  }

  function updateCalculator(value, action) {
    if (action === 'clear') { calculatorInput = ''; calculatorLastValue = ''; renderCalculator(); return; }
    if (action === 'backspace') { calculatorInput = calculatorInput.slice(0, -1); calculatorLastValue = ''; renderCalculator(); return; }
    if (action === 'equals') { evaluateCalculator(); return; }
    if (action === 'parenthesis') {
      const opens = (calculatorInput.match(/\(/g) || []).length;
      const closes = (calculatorInput.match(/\)/g) || []).length;
      value = opens > closes && /[0-9πe)%]/.test(calculatorInput.slice(-1)) ? ')' : '(';
    }
    if (!value) return;
    if (calculatorLastValue && /^(?:[0-9.(πe]|(?:sin|cos|tan|asin|acos|atan|sqrt|ln|log)\()/.test(value)) calculatorInput = '';
    calculatorLastValue = '';
    if (calculatorInput.length >= 120) return;
    calculatorInput += value;
    const result = calculateCurrentExpression();
    renderCalculator(result.ok ? result : null);
  }

  function setTranslatorStatus(message, error = false) {
    [translatorStatus, homeTranslatorStatus].filter(Boolean).forEach((element) => {
      element.textContent = message;
      element.classList.toggle('error', error);
    });
  }

  async function translateText(source = 'tools') {
    const fromHome = source === 'home';
    const sourceInput = fromHome ? homeTranslatorSource : translatorSource;
    const targetInput = fromHome ? homeTranslatorTarget : translatorTarget;
    const engineInput = fromHome ? homeTranslatorEngine : translatorEngine;
    const submit = fromHome ? homeTranslatorSubmit : translatorSubmit;
    const text = String(sourceInput?.value || '').trim();
    if (!text) { setTranslatorStatus('先输入要翻译的文字。', true); sourceInput?.focus(); return; }
    if (!window.notchAPI?.translateText) { setTranslatorStatus('翻译服务不可用。', true); return; }
    [translatorSubmit, homeTranslatorSubmit].filter(Boolean).forEach((button) => { button.disabled = true; });
    setTranslatorStatus('正在翻译…');
    if (translatorResultMeta) translatorResultMeta.textContent = '正在处理';
    const targetLanguage = targetInput?.value || 'en';
    const engine = engineInput?.value || 'auto';
    const result = await window.notchAPI.translateText({ text, targetLanguage, engine })
      .catch(() => ({ ok: false, error: 'request_failed' }));
    [translatorSubmit, homeTranslatorSubmit].filter(Boolean).forEach((button) => { button.disabled = false; });
    if (!result?.ok) {
      const message = result?.error === 'not_configured'
        ? '请先配置在线 API，或安装本地翻译模型。'
        : result?.error === 'local_model_missing'
          ? '本地翻译模型未安装，请先安装对应语言包。'
          : result?.error === 'local_unavailable'
            ? '本地翻译不可用，请安装 Argos Translate。'
            : result?.error === 'timeout' ? '翻译超时，请重试。' : '翻译失败，请检查翻译引擎配置。';
      setTranslatorStatus(message, true);
      if (translatorResultMeta) translatorResultMeta.textContent = '未完成';
      return;
    }
    translatorText = result.text;
    if (translatorResult) translatorResult.textContent = translatorText;
    if (homeTranslatorResult) homeTranslatorResult.textContent = translatorText;
    if (translatorCopy) translatorCopy.disabled = false;
    if (fromHome && translatorSource && document.activeElement !== translatorSource) translatorSource.value = text;
    if (!fromHome && homeTranslatorSource && document.activeElement !== homeTranslatorSource) homeTranslatorSource.value = text;
    if (translatorTarget) translatorTarget.value = targetLanguage;
    if (homeTranslatorTarget) homeTranslatorTarget.value = targetLanguage;
    const usedLocal = result.engine === 'local';
    if (translatorResultMeta) translatorResultMeta.textContent = usedLocal ? '本地离线完成' : '在线 API 完成';
    setTranslatorStatus(usedLocal ? '已使用本地离线翻译。' : '在线翻译完成。');
  }

  calculatorKeypad?.addEventListener('click', (event) => {
    const button = event.target.closest('button');
    if (!button) return;
    updateCalculator(button.dataset.calculatorValue, button.dataset.calculatorAction);
  });
  calculatorAngleButtons.forEach((button) => {
    button.addEventListener('click', () => setCalculatorAngleMode(button.dataset.calculatorAngle));
  });
  document.addEventListener('keydown', (event) => {
    if (document.activeElement?.matches('input, textarea, select')) return;
    if (!document.getElementById('tab-tools')?.classList.contains('active')) return;
    if (/^[0-9+*/%^.()]$/.test(event.key)) { event.preventDefault(); updateCalculator(event.key); }
    else if (event.key === '-') { event.preventDefault(); updateCalculator('−'); }
    else if (event.key === 'Enter' || event.key === '=') { event.preventDefault(); updateCalculator('', 'equals'); }
    else if (event.key === 'Backspace') { event.preventDefault(); updateCalculator('', 'backspace'); }
    else if (event.key === 'Escape') { event.preventDefault(); updateCalculator('', 'clear'); }
  });
  translatorSubmit?.addEventListener('click', () => translateText('tools'));
  translatorSource?.addEventListener('keydown', (event) => {
    if ((event.metaKey || event.ctrlKey) && event.key === 'Enter') { event.preventDefault(); translateText('tools'); }
  });
  [translatorEngine, homeTranslatorEngine].filter(Boolean).forEach((select) => {
    select.addEventListener('change', () => {
      [translatorEngine, homeTranslatorEngine].filter(Boolean).forEach((other) => { other.value = select.value; });
      setTranslatorStatus(select.value === 'local' ? '将使用本地离线翻译。' : select.value === 'online' ? '将使用在线 API。' : '自动优先使用本地翻译。');
    });
  });
  [translatorTarget, homeTranslatorTarget].filter(Boolean).forEach((select) => {
    select.addEventListener('change', () => {
      [translatorTarget, homeTranslatorTarget].filter(Boolean).forEach((other) => { other.value = select.value; });
    });
  });
  homeCalculatorSubmit?.addEventListener('click', () => {
    calculatorInput = String(homeCalculatorInput?.value || '').trim();
    calculatorLastValue = '';
    evaluateCalculator();
  });
  homeCalculatorInput?.addEventListener('keydown', (event) => {
    if (event.key === 'Enter') {
      event.preventDefault();
      calculatorInput = String(homeCalculatorInput.value || '').trim();
      calculatorLastValue = '';
  ...
  homeTranslatorSubmit?.addEventListener('click', () => translateText('home'));
  homeTranslatorSource?.addEventListener('keydown', (event) => {
    if ((event.metaKey || event.ctrlKey) && event.key === 'Enter') { event.preventDefault(); translateText('home'); }
  });
  translatorCopy?.addEventListener('click', async () => {
    if (!translatorText || !navigator.clipboard?.writeText) return;
    await navigator.clipboard.writeText(translatorText).then(() => setTranslatorStatus('已复制翻译结果。')).catch(() => setTranslatorStatus('复制失败，请手动复制。', true));
  });

  if (window.notchAPI?.getTranslationStatus) {
    window.notchAPI.getTranslationStatus().then((status) => {
      const message = status?.localAvailable
        ? '本地翻译可用 · 自动模式将优先离线翻译'
        : '未检测到本地翻译 · 可安装 Argos Translate 或使用在线 API';
      setTranslatorStatus(message, false);
    }).catch(() => {});
  }
  ...
  settingsHomeLayoutPreset?.addEventListener('change', () => {
    const previous = window.NotchHome?.getLayoutPreset?.() || 'free';
    const result = window.NotchHome?.setLayoutPreset?.(settingsHomeLayoutPreset.value);
    if (!result?.ok) {
      settingsHomeLayoutPreset.value = previous;
      if (typeof showStatusToast === 'function') showStatusToast('首页排布未更新，请重试');
      return;
    }
    if (typeof showStatusToast === 'function') {
      showStatusToast(result.persisted === false ? '排布已更新，仅当前会话生效' : result.preset === 'free' ? '已切换为自由排布' : '首页排布已更新');
    }
    renderHomeModuleSettings();
  });
  ...
  document.addEventListener('notch:home-layout-preset-changed', renderHomeModuleSettings);
