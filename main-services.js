...
function isValidPanelShortcutKey(key) {
  if (typeof key !== 'string') return false;
  return /^(?:[A-Z0-9]|F(?:[1-9]|1[0-9]|2[0-4])|Space|Tab|Escape|Left|Right|Up|Down|Home|End|PageUp|PageDown|Backspace|Delete|Enter|Backquote)$/.test(key);
}
...
const CONFIGURABLE_FEATURES = new Set(['todo', 'notes', 'links', 'recordings', 'credentials', 'clip', 'tools']);
const DEFAULT_PANEL_TABS = new Set(['home', 'todo', 'notes', 'links', 'recordings', 'credentials', 'clip', 'tools', 'settings']);
...
// 网易云音乐使用应用内播放快捷键；播放/暂停必须共用 Space，切歌使用 Cmd+方向键。
// 通过 System Events 前置应用后发送，避免依赖不稳定的媒体会话元数据。
