...
const HOME_MODULES = ['music', 'pomodoro', 'recorder', 'windows', 'mirror', 'note', 'commands', 'calculator', 'translator'];
...
test('settings stays at the far right when optional tabs are hidden', () => {
  ...
  const tabs = ['home', 'todo', 'notes', 'links', 'recordings', 'credentials', 'tools', 'clip', 'settings'];
  assert.deepEqual(visiblePanelTabs(tabs, { todo: false, clip: true }), [
    'home', 'notes', 'links', 'recordings', 'credentials', 'tools', 'clip', 'settings',
  ]);
...
test('calculator parses safe arithmetic and scientific expressions', () => {
  assert.deepEqual(calculateExpression('2 + 3 × (4 - 1)'), { ok: true, value: 11, display: '11' });
  assert.deepEqual(calculateExpression('50% + .25'), { ok: true, value: 0.75, display: '0.75' });
  assert.deepEqual(calculateExpression('2^8'), { ok: true, value: 256, display: '256' });
  assert.deepEqual(calculateExpression('2π'), { ok: true, value: 6.28318530718, display: '6.28318530718' });
  assert.deepEqual(calculateExpression('√(16) + log(100) + ln(e)'), { ok: true, value: 7, display: '7' });
  assert.deepEqual(calculateExpression('sin(30)', { angleMode: 'deg' }), { ok: true, value: 0.5, display: '0.5' });
  assert.deepEqual(calculateExpression('sin(1.5707963267948966)', { angleMode: 'rad' }), { ok: true, value: 1, display: '1' });
  assert.deepEqual(calculateExpression('asin(0.5)', { angleMode: 'deg' }), { ok: true, value: 30, display: '30' });
  assert.equal(calculateExpression('1 / 0').error, 'divide_by_zero');
  assert.equal(calculateExpression('sqrt(-1)').error, 'domain_error');
  assert.equal(calculateExpression('asin(2)').error, 'domain_error');
  assert.equal(calculateExpression('2 + alert(1)').ok, false);
  assert.equal(calculateExpression('Math.sin(1)').ok, false);
});
...
test('homepage visibility refuses to hide the final visible module', () => {
  const allButCommandsHidden = HOME_MODULES.filter((id) => id !== 'commands');
  assert.deepEqual(
    updateHomeModuleVisibility(allButCommandsHidden, HOME_MODULES, 'commands', false),
    { ok: false, error: 'at_least_one_required', hiddenIds: allButCommandsHidden }
  );
  assert.deepEqual(
