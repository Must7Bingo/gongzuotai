...
test('Windows strip avoids a top taskbar and expanded panel stays in usable desktop', () => {
  ...
  assert.deepEqual(platform.panelBounds('win32', display, true), { x: -1520, y: -152, width: 1120, height: 616 });
});
...
test('Mac retains notch height and physical top origin', () => {
  ...
  assert.deepEqual(platform.panelBounds('darwin', display, true), { x: 196, y: 0, width: 1120, height: 616 });
});
