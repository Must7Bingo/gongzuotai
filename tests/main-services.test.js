...
test('panel shortcut key validation accepts the Backquote accelerator token', () => {
  assert.equal(isValidPanelShortcutKey('Backquote'), true);
  assert.equal(isValidPanelShortcutKey('Space'), true);
  assert.equal(isValidPanelShortcutKey('Left'), true);
  assert.equal(isValidPanelShortcutKey('Control'), false);
});
...
test('feature preferences only update configurable tabs and keep permanent tabs enabled', () => {
  ...
  assert.deepEqual(updateFeaturePreference({ tools: false }, 'tools', true), { tools: true, home: true });
...
test('default panel tab accepts visible tabs and falls back to home safely', () => {
  ...
  assert.equal(normalizeDefaultTabPreference('tools', { tools: true }), 'tools');
...
test('running NetEase Music uses its own shortcuts because native media status can stay empty', async () => {
  const events = [];
  const result = await controlNetEaseMusic('play', {
...
test('NetEase Music play uses the play/pause toggle instead of the next-track key', () => {
