...
async function main() {
  ...
  try {
    ...
    for (const [width, height] of [[1120, 616], [1000, 576]]) {
    ...
    const lifecycleAudit = await window.webContents.executeJavaScript(`
      (async () => {
        ...
        await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
        const musicTile = document.getElementById('home-music');
        const mirrorTile = document.querySelector('[data-home-module="mirror"]');
        musicTile.dispatchEvent(new PointerEvent('pointerenter', { clientX: 30, clientY: 30 }));
        musicTile.dispatchEvent(new PointerEvent('pointermove', { clientX: 58, clientY: 40 }));
        await new Promise((resolve) => requestAnimationFrame(resolve));
        const musicLiquidActive = musicTile.classList.contains('liquid-hover')
          && musicTile.querySelector('.tile-liquid-surface') !== null;
        mirrorTile.dispatchEvent(new PointerEvent('pointerenter', { clientX: 80, clientY: 80 }));
        await new Promise((resolve) => requestAnimationFrame(resolve));
        const mirrorLiquidActive = mirrorTile.classList.contains('liquid-hover')
          && mirrorTile.querySelector('.tile-liquid-surface') !== null;
        musicTile.dispatchEvent(new PointerEvent('pointerleave'));
        await new Promise((resolve) => requestAnimationFrame(resolve));
        const musicLiquidCleared = !musicTile.classList.contains('liquid-hover');
    ...
    assert.equal(lifecycleAudit.musicLiquidActive, true);
    assert.equal(lifecycleAudit.mirrorLiquidActive, true);
    assert.equal(lifecycleAudit.musicLiquidCleared, true);
    ...
    const idlePerformanceAudit = await window.webContents.executeJavaScript(`
      (async () => {
        ...
        document.dispatchEvent(new CustomEvent('notch:modechange', { detail: { expanded: true } }));
        await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
        const hasMusicCanvas = document.getElementById('music-color-bends') !== null;
    ...
    `);
    assert.equal(idlePerformanceAudit.hasMusicCanvas, false, '首页不应保留音乐组件独占的 WebGL 画布');
